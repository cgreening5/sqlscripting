...
from parsing.expressions.datatype import DataTypeClause, TextDataTypeClause
from parsing.expressions.declare_expression import DeclareTableVariableExpression, DeclareVariableExpression
from parsing.expressions.table_definition import ColumnDefinitionExpression, TableDefinitionExpression
...
class TestParser(unittest.TestCase):
    ...
    def test_declare_table_clause(self):
        ...
        self.assertEqual(len(parsed_clauses), 1)
        declare_clause = parsed_clauses[0]
        self.assertIsInstance(declare_clause, DeclareTableVariableExpression)
        ...
        column: ColumnDefinitionExpression = table_def.columns[0]
        self.assertEqual(column.name.token.value, 'id')
        self.assertEqual(column.datatype.datatype.token.value, 'INT')
    ...
    def test_declare_table_with_text_datatype(self):
        ...
        self.assertIsInstance(declare_clause, DeclareTableVariableExpression)
        self.assertEqual(declare_clause.name.token.value, '@var')
        table_def: TableDefinitionExpression = declare_clause.table_definition
        self.assertEqual(len(table_def.columns), 1)
        column: ColumnDefinitionExpression = table_def.columns[0]
        self.assertEqual('description', column.name.token.value)
        self.assertIsInstance(column.datatype, TextDataTypeClause)
        self.assertEqual(column.datatype.datatype.token.value, 'NVARCHAR')
        self.assertEqual(column.datatype.length.token.value, '100')